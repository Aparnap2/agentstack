...
# Version: 2.0.0 (Adapted for AgentStack OSS)
...
if ! command -v nc >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
    echoerr "Required tools not found: nc or timeout"
    exit 1
fi

# Determine if netcat is in busybox mode
if command -v nc >/dev/null 2>&1; then
    if nc -l 0 >/dev/null 2>&1; then
        WAITFORIT_ISBUSY=0
    else
        WAITFORIT_ISBUSY=1
    fi
